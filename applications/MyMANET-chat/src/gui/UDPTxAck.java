...
public class UDPTxAck {
    ...
    public UDPTxAck(UnifiedForm f, String srcIPv4, String destIPv4, int type, long seq, String pay_load, int destPort, int srcPort, int ticks, int tOut) {
        loadObjectData(f, srcIPv4, destIPv4, type, seq, pay_load.getBytes(), destPort, srcPort, ticks, tOut);
    }

    public UDPTxAck(UnifiedForm f, String srcIPv4, String destIPv4, int type, long seq, byte[] pay_load, int destPort, int srcPort, int ticks, int tOut) {
        loadObjectData(f, srcIPv4, destIPv4, type, seq, pay_load, destPort, srcPort, ticks, tOut);
    }

    /* Unit testing done */
    private void loadObjectData(UnifiedForm f, String srcIPv4, String destIPv4, int type, long seq, byte[] pay_load, int destPort, int srcPort, int ticks, int tOut) {
        form = f;
        srcIP = srcIPv4;
        destIP = destIPv4;
        mType = type;
