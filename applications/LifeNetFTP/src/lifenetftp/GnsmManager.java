...
public class GnsmManager {
    ...
    public int getHostIndex(String name, String ipAdd, String macAdd, int flag) {
        ...
        closeLastOpenedFile();
        FTPDebug.printDebug("\nInside gethostindex args " + name + " return " + index, Main.DEBUG);
        return index;
    }
        ...
        if (index1 != -1 && index2 != -1) {
            ...
            if (getHostCount() != 0) {
                if (index1 != -1 && index2 != -1) {
                    ...
                    FTPDebug.printDebug("Get Tx Session args " + index1 + " " + index2 + "calc line number " + line_number, Main.DEBUG);
    ...
    public int getVD(String name1, String name2) {
        FTPDebug.printDebug("Inside getVD", Main.DEBUG);
        if (getHostIndex(name1, null, null, SEL_NAME) != -1 && getHostIndex(name2, null, null, SEL_NAME) != -1) {
            FTPDebug.printDebug("Inside getVD " + name1 + " " + name2 + " returning " + getVD(getHostIndex(name1, null, null, SEL_NAME), getHostIndex(name2, null, null, SEL_NAME)), Main.DEBUG);
            return getVD(getHostIndex(name1, null, null, SEL_NAME), getHostIndex(name2, null, null, SEL_NAME));
        } else {
            FTPDebug.printDebug("Inside getVD " + name1 + " " + name2 + " returning -1", Main.DEBUG);
            return -1;
        }
    }
    ...
    private boolean openFile(int fileMode, String filename) {
        ...
        try {
            ...
            if (Main.DEBUG) {
                FTPDebug.printDebug("File not found exception in openfile", Main.DEBUG);
                ex.printStackTrace();
            ...
            if (Main.DEBUG) {
                FTPDebug.printDebug("IO Exception in openfile", Main.DEBUG);
                Logger.getLogger(
    ...
    private String[] readIntoArray() {
        ...
        try {
            ...
            if (Main.DEBUG) {
                ...
                FTPDebug.printDebug(
                        "File not found in readintoarray", Main.DEBUG);
                ex.printStackTrace();
            }
            ...
            if (Main.DEBUG) {
                FTPDebug.printDebug("Null ptr exception caught in readintoarray", Main.DEBUG);
                ex.printStackTrace();
    ...
    private void closeLastOpenedFile() {
        try {
            ...
            if (Main.DEBUG) {
                FTPDebug.printDebug("inside closefile", Main.DEBUG);
                Logger.getLogger(
                        FileHandler.class.getName()).log(Level.SEVERE, null, ex);
                ex.printStackTrace();
            }
            ...
            if (Main.DEBUG) {
                FTPDebug.printDebug("Null ptr. exception in closefile", Main.DEBUG);
                exc.printStackTrace();
