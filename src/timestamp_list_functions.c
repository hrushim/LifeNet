...
int add_or_update_timestamp_entry(uint8_t mac[6], uint32_t timestamp){
	...
	if(timestamp_head == NULL){
		...
		if(search_and_update_timestamp(mac, timestamp) == 1){
 ...
 * return 0
 * */
uint8_t search_and_update_timestamp(uint8_t orig_sender[6], uint32_t new_timestamp){
